...
class Planta:
    ...
    def Print_Maq_Res(self,ref,maq):
        if(self.log):
            self.writer.add_scalars(f'Produção/'+ref,
                                         {'sp':maq.get('sp',0),
                                          'wp':maq.get('wp',0),
                                          'wt':maq.get('wt',0),
                                          'bp':maq.get('bp',0),
    ...
    def Get_RC(self,done):
        ...
        for maquina in estado:  
            for item in maquina:
                ...
                if len(self.last_perfs) < self.n_maquinas :
                    print('ciclo 0')
                    self.last_perfs.append([wp,wt,bp,sp,ot])
                ...
                if(item[0] == 'M'):
                    ...
                    delta_Wp = wp - self.last_perfs[index_m][0]
                    delta_Wt = wt - self.last_perfs[index_m][1]
                    delta_Bp = bp - self.last_perfs[index_m][2]
                    delta_Sp = sp - self.last_perfs[index_m][3]
                    delta_Ot = ot - self.last_perfs[index_m][4]
                ...
                at_perfs.append([wp,wt,bp,sp,ot])

        self.last_perfs = at_perfs
        rc += total_bf_rw

        if(self.num_Exits < 0.85*self.maxProd): # avaliar punição mais abruptas
            rc += -1.0
        else:
            rc += +1.0#(self.num_Exits/self.maxProd)
    ...
    def Exec_action(self, acao):
        ...
        for it in acao_exec:
            ...
            self.maquinas[i].Set_Vel(vel_n)
            if(self.log):
                self.writer.add_scalar("Vel_m/"+self.maquinas[i].machine.name, vel_n,self.itc_total )
            i+=1
    ...
    def step(self,action):
        ...
        if(self.log):
            self.writer.add_scalar("RC step", pontos,self.itc_total )
            self.writer.add_scalar("Produção", (100.0*self.num_Exits/self.maxProd), self.itc_total )
            self.writer.add_scalar("RC total", self.toral_rc, self.itc_total )
            self.writer.add_scalar("Podução", self.num_Exits,self.itc_total )
