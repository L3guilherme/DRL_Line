...
#                 self.printTrace(self.id, attemptSignalReceiver='(removeEntity)')
    ...
    def Get_Status(self,tipo = 'abs'):

        level = float(len(self.Res.users))
        if tipo == 'abs':
            return {self.name:{'lv':level}}
        elif tipo == 'dis':
            uso = level/float(self.capacity)          
            if uso < 0.33:
                return {self.name:{'lv':0.0}}
            elif uso >= 0.33 and uso < 0.66:
                return {self.name:{'lv':0.5}}
            elif uso >= 0.66:
                return {'lv':1}
        return {self.name:{'lv':level/float(self.capacity)}}
