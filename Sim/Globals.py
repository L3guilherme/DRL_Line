...
#    # this is added for backwards compatibility
    if dotted_name.startswith(dotted_name):
        class_name = dotted_name#.split('.')[-1]
        new_dotted_name = '%s.%s' % (class_name, class_name)
...
def stepSimulation(objectList=[], maxSimTime=100,numberOfReplications=1,trace='No', seed=1):
